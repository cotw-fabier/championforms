...
class CheckboxSelect extends OptionSelect {
  ...
  /// Builds a [CheckboxSelect] from its JSON map.
  ///
  /// Registered for the type names `checkboxSelect` and `checkbox`. Both are
  /// this class, and `multiselect` is what separates them: `checkbox` defaults
  /// to a single-select group (ticking one clears the others), `checkboxSelect`
  /// to a multi-select. An explicit `"multiselect"` in the document wins either
  /// way.
  ///
  /// Recognised keys are the same as [OptionSelect.fromJson].
  factory CheckboxSelect.fromJson(Map<String, dynamic> json) {
    ...
    return CheckboxSelect(
      ...
      options: options,
      multiselect: FieldJson.boolean(
        json,
        'multiselect',
        fallback: FieldJson.type(json) == 'checkboxSelect',
      ),
